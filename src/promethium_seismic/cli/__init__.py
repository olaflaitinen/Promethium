...
Usage:
    promethium_seismic run --input data.sgy --pipeline matrix_completion --output
    result.sgy
    promethium_seismic evaluate --reference truth.npy --estimate recon.npy
    promethium_seismic datasets list
    promethium_seismic models list
"""
...
def run(
    input_path: Path = typer.Argument(
        ..., help="Path to input seismic data (SEG-Y, HDF5, NumPy)"
    ),
    output_path: Path = typer.Argument(..., help="Path for output reconstructed data"),
    pipeline: str = typer.Option(
        "matrix_completion", "--pipeline", "-p", help="Pipeline preset name"
    ),
    config: Path | None = typer.Option(
        None, "--config", "-c", help="Path to YAML configuration file"
    ),
    lambda_param: float = typer.Option(
        0.1, "--lambda", help="Regularization parameter"
    ),
    ...
    console.print(
        f"Loaded: {dataset.traces.shape[0]} traces, {dataset.traces.shape[1]} samples"
    )
...
def evaluate(
    ...
    metrics: str = typer.Option(
        "snr,mse,psnr,ssim", "--metrics", "-m", help="Comma-separated list of metrics"
    ),
...
def ingest(
    input_path: Path = typer.Argument(
        ..., help="Path to raw seismic file(s) or directory"
    ),
    output_dir: Path = typer.Argument(..., help="Output directory for processed data"),
    format: str = typer.Option(
        "hdf5", "--format", "-f", help="Output format: hdf5, npy, zarr"
    ),
...
def batch_run(
    config_dir: Path = typer.Argument(
        ..., help="Directory containing pipeline config files"
    ),
    output_dir: Path = typer.Option(
        Path("results"), "--output", "-o", help="Output directory"
    ),
    experiment_id: str | None = typer.Option(
        None, "--experiment-id", help="Experiment ID for logging"
    ),
...
def experiments_list(
    logs_dir: Path = typer.Option(
        Path("experiments/logs"), "--dir", "-d", help="Logs directory"
    ),
):
...
def experiments_show(
    ...
    for run in runs[-last_n:]:
        ...
        table.add_row(
            ...
            f"{metrics.get('snr', 'N/A'):.2f}"
            if isinstance(metrics.get("snr"), (int, float))
            else "N/A",
            f"{metrics.get('mse', 'N/A'):.2e}"
...
def experiments_export(
    ...
    output: Path = typer.Option(
        Path("experiment_export.csv"), "--output", "-o", help="Output file"
    ),
    format: str = typer.Option(
...
def version():
    ...
    from promethium_seismic import __version__
