...
def matrix_completion_ista(
    M: np.ndarray, mask: np.ndarray, lambda_: float = 0.1, max_iter: int = 50
) -> np.ndarray:
...
def main():
    ...
    mc_metrics = {
        "relative_error": float(
            np.linalg.norm(completed - true_matrix) / np.linalg.norm(true_matrix)
        ),
