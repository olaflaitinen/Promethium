...
class PhysicsInformedNN(nn.Module):
    ...
    Where Loss_physics is the residual of the wave equation,
    u_tt - c^2 * (u_xx + u_zz) = 0
    """

    def __init__(self, layers=None):
        # A list literal in a default is built once and shared by every
        # instance, so a caller that mutates it changes the default for
        # everyone afterwards. The sentinel is None and the real default
