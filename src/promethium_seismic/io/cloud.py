...
class CloudAdapter:
    ...
    def __init__(
        self, provider: str, bucket_name: str, connection_string: str | None = None
    ):
        self.provider = provider.lower()
        self.bucket_name = bucket_name
        self.connection_string = connection_string or os.getenv(
            "CLOUD_STORAGE_CONN_STR"
        )
    ...
    def _initialize_client(self):
        if self.provider == "s3":
            ...
            try:
                ...
                print(
                    "Warning: azure-storage-blob not installed. "
                    "Azure features unavailable."
                )
    ...
    def upload_file(self, file_path: str, object_name: str | None = None):
        ...
        if self.provider == "s3" and self.s3_client:
            ...
            blob_client = self.blob_service_client.get_blob_client(
                container=self.bucket_name, blob=object_name
            )
            ...
            raise NotImplementedError(
                f"Provider {self.provider} not fully initialized or supported."
            )
