...
class CompressiveSensing(RecoveryAlgorithm):
    """
    Compressive Sensing recovery using Iterative Shrinkage-Thresholding Algorithm
    (ISTA).
    ...
    def fit(self, data: np.ndarray, mask: np.ndarray = None) -> "CompressiveSensing":
        ...
        # Here, A is the Sampling Operator (Phi) combined with Inverse Transform
        # (Psi^-1) if we solve for coefficients.
